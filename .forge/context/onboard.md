...
| 模块 | 路径 | 职责 |
|------|------|------|
| Marketplace manifest | `.claude-plugin/marketplace.json` | 声明插件位于 `./plugins/forge`，供 Claude Code 安装 |
| Plugin manifest | `plugins/forge/.claude-plugin/plugin.json` | 插件元数据（name、version、description） |
| **forge orchestrator** | `plugins/forge/skills/forge/` | 主编排器：运行 `status.mjs` 检测状态，路由到正确子 skill |
| onboard skill | `plugins/forge/skills/onboard/` | 扫描项目，生成人可读的项目地图 |
| calibrate skill | `plugins/forge/skills/calibrate/` | 提取代码约定，交互式裁决矛盾，产出 4 个 context 文件 |
| clarify skill | `plugins/forge/skills/clarify/` | 追踪代码路径，澄清需求，提取未知项（调用 forge-explorer） |
| design skill | `plugins/forge/skills/design/` | 并发探索技术方向，产出设计方案（调用 forge-architect） |
| tasking skill | `plugins/forge/skills/tasking/` | 将设计拆解为带验收标准的有序任务列表 |
| code skill | `plugins/forge/skills/code/` | 严格按任务范围实现，含 Scope Creep 和 New Pattern 协议 |
| inspect skill | `plugins/forge/skills/inspect/` | 并发逐文件评审，置信度 ≥80 才上报（调用 forge-reviewer） |
| test skill | `plugins/forge/skills/test/` | 生成符合项目测试规范的测试代码和测试计划 |
| forge-explorer agent | `plugins/forge/agents/forge-explorer.md` | 代码路径追踪（🟡），由 clarify 并发调用 |
| forge-architect agent | `plugins/forge/agents/forge-architect.md` | 技术方向设计（🟢），由 design 并发调用 |
| forge-reviewer agent | `plugins/forge/agents/forge-reviewer.md` | 逐文件评审（🔴），由 inspect 并发调用 |
| Status script | `plugins/forge/skills/forge/scripts/status.mjs` | 唯一权威状态检测器，输出 `[ACTION]` 路由指令 |
| Calibrate scripts | `plugins/forge/skills/calibrate/scripts/` | 前置检查、扫描状态保存、产物校验（3 个脚本） |
| Reference docs | `plugins/forge/skills/*/reference/` | 各 skill 的输出模板和参考文档 |
