...
| `plugins/forge/skills/onboard/reference/kinds/plugin.md` | skill reference | kind 定义 | 检测 `.claude-plugin/` 结构；使用 plugin 专属 profile |
...
| `structural/` | `codebase-structure-plugin.md` | plugin | Skills/Agents/Scripts/Docs 视角 |
...
| `model/` | `artifact-types.md` | plugin | Artifact 类型 + 生产者/消费者关系 |
| `entry-points/` | `http-entry-points.md` | web-backend | Controller 路由 + 调用链 |
| `entry-points/` | `slash-command-entry-points.md` | plugin | Slash command 列表 + 执行链 |
...
| Profile | web-backend | plugin | monorepo |
|---------|:-----------:|:------------------:|:--------:|
