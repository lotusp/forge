...
本 run 的执行计划（有序 profile 序列）
  ...
  ↓   - 可选 opt-in 质量模式：每 section 写后触发 self-critique round-trip
...
| **DG3** | 生成质量高 | IRON RULES 完备；confidence tag 强制；verification pass；opt-in 质量模式启用 self-critique；profile 自带 golden-path 示例 |
...
4. **Kind 显式覆盖能力：** 用户可显式指定 kind 绕过自动检测；按指定 kind 的
   profile 序列执行；无效指定给出清晰错误提示与可选值列表。（具体激活方式
   由 design 决定）
5. **Opt-in 质量模式：** 用户可显式激活 self-critique 质量模式；启用后每个
   section 写入后看到至少一次自检 round-trip；默认关闭不产生此开销。
   （具体激活方式由 design 决定）
6. **低置信度提示：** 当 kind detection 的 primary 与次名得分接近（阈值由
   design 决定），skill 主动提示用户确认，并提供"显式覆盖"的操作指引。
...
合并原 Assumptions 与 Open Questions。以下为**需用户介入**的议题（需求 / 交互边界层面）。
纯实现选择（cross-reference 语法、并行策略等）不列入 OQ，由 design 阶段直接裁决。

| # | 议题 | 当前默认假设 | Impact if unresolved |
|---|------|-------------|----------------------|
| OQ-01 | Thorough 质量模式在 incremental 模式下的语义（是全量刷所有 section，还是只对重写 section 做 self-critique？） | 假设与 first-run 一致（全量 + self-critique） | Run Mode 矩阵不完整；影响用户可预期性 |
| OQ-02 | Kind detection 低置信度触发用户确认的**具体阈值**（primary 与次名的得分差百分比） | 假设差距 < 20% 触发确认 | 影响交互频率：过敏 = 每次烦用户；过钝 = 误检默认吞 |
| OQ-03 | 哪些 profile 归为"所有 kind 共用的核心"（例如 project-identity / local-development / known-traps / document-confidence） | 假设 5 个核心 profile | 影响 profile 库组织；影响新 kind 文件的最低 profile 列表 |
...
| **Gap-06** | Kind 显式覆盖能力 + opt-in 质量模式 — 两个新用户激活机制的解析与路由 | 用户显式控制 |
